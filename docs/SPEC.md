...
│ [期限/達成日]                 │
├────────────────────────────────┤
│             [編集] [達成/元に戻す]│ ← 右下にアクションボタン
└────────────────────────────────┘
```

**達成済みの取り消し**:
- 達成済みカードには「元に戻す」ボタンを表示
- 取り消し時は条件を再評価して適切なステータス（pending/achievable）に戻す
...
|------|------|
| 2025-01-25 | 初版作成 |
| 2026-01-28 | 日記保存時のフィードバックをトースト通知に変更 |
